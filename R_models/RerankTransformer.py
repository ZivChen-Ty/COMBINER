...
class RerankTransformer(nn.Module):
    def __init__(self, topk, topk_dim=512, embed_dim=2048, depth=6, num_heads=4, mlp_ratio=4., \
        ...
        CIR_Dim =  512
        self.ln = nn.LayerNorm((topk_dim - CIR_Dim) // 2, elementwise_affine=True)
        self.ln1 = nn.LayerNorm((topk_dim - CIR_Dim)  // 2, elementwise_affine=True)
    ...
    def _init_weights(self, m):     #自定义初始化权重；
    ...
    def forward_embedding(self, x_topk: Tensor, A_mat: Tensor, topk_indices: Tensor):
        ...
        CIR_dim = 512
        x_CIR = x_topk[:, : , - CIR_dim:]
        x_topk_inter, x_topk_intra = x_topk[:, : , :-CIR_dim].chunk(2, dim=-1)

        x_topk_inter, x_topk_intra = self.ln(x_topk_inter), self.ln1(x_topk_intra)
        x_topk = torch.cat([x_topk_inter, x_topk_intra, x_CIR], dim=-1)
